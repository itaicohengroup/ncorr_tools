...
%
% ***
% Modified ncorr 1.2.1
% Modifications created by: Lena R. Bartell, lrb89@cornell.edu
% Priniciple Investigator: Prof. Itai Cohen, Cornell University
% Notes: Added lines to save displacement gradient data in the
%   "data_dic_save" output. Changes/additions are marked with the text
%   "Lena Bartell edit" in a comment.
% Date: 2/24/2017
% ***
%
%-------------------------------------------------------------------------%
    ...
    methods(Access = public)
        ...
        function constructor(obj)      
            ...
            obj.data_dic = struct('displacements',struct('plot_u_dic',{}, ...                                                % U plot after DIC (can be regular or eulerian)
                                  ...
                                  'deformgrad',struct('plot_dudx_ref_formatted',{}, ...                                      % dudx reference (Green-Lagrange) Lena Bartell edit
                                                      'plot_dudy_ref_formatted',{}, ...                                      % dudy reference (Green-Lagrange) Lena Bartell edit
                                                      'plot_dvdx_ref_formatted',{}, ...                                      % dvdx reference (Green-Lagrange) Lena Bartell edit
                                                      'plot_dvdy_ref_formatted',{}, ...                                      % dvdy reference (Green-Lagrange) Lena Bartell edit
                                                      'plot_dudx_cur_formatted',{}, ...                                      % dudx current (Eulerian-Almansi) Lena Bartell edit
                                                      'plot_dudy_cur_formatted',{}, ...                                      % dudy current (Eulerian-Almansi) Lena Bartell edit
                                                      'plot_dvdx_cur_formatted',{}, ...                                      % dvdx current (Eulerian-Almansi) Lena Bartell edit
                                                      'plot_dvdy_cur_formatted',{}), ...                                     % dvdy current (Eulerian-Almansi) Lena Bartell edit
    ...
    methods(Access = private)
                    ...
                    if (isfield(struct_load,'reference_save') && isfield(struct_load,'current_save') && isfield(struct_load,'data_dic_save'))
                        ...
                        if (isa(struct_load.reference_save,'struct') && ...
                            ...
                            try
                                ...
                                % try again w/o the deformgrad
                                data_dic_prelim = rmfield(data_dic_prelim, 'deformgrad');
                                try
                                    data_dic_prelim(1) = struct_load.data_dic_save; %#ok<NASGU>
                                    loaddatasuccess = true;
                                catch
                                    % still have incorrect field matches
                                ...
                                if (overwrite)     
                                    ...
                                    try               
                                        ...
                                        if (outstate_ref == out.success && outstate_cur == out.success)        
                                            ...
                                            % Set dic data:
                                            tmp_fields = fieldnames(struct_load.data_dic_save);
                                            for ii = 1:length(tmp_fields)
                                                obj.data_dic(1).(tmp_fields{ii}) = struct_load.data_dic_save.(tmp_fields{ii});
                                            end
                                        else
        ...
        % displacements have been formatted.
            ...
            for i = 0:length(obj.current)-1
                ...
                deformgrad_template(i+1).plot_dudx_ref_formatted = plots_dispgrad_ref_prelim(i+1).plot_dudx; %#ok<AGROW> % lena bartell edit
                deformgrad_template(i+1).plot_dudy_ref_formatted = plots_dispgrad_ref_prelim(i+1).plot_dudy; %#ok<AGROW> % lena bartell edit
                deformgrad_template(i+1).plot_dvdx_ref_formatted = plots_dispgrad_ref_prelim(i+1).plot_dvdx; %#ok<AGROW> % lena bartell edit
                deformgrad_template(i+1).plot_dvdy_ref_formatted = plots_dispgrad_ref_prelim(i+1).plot_dvdy; %#ok<AGROW> % lena bartell edit
                ...
                deformgrad_template(i+1).plot_dudx_cur_formatted = plots_dispgrad_cur_prelim(i+1).plot_dudx; %#ok<AGROW> % lena bartell edit
                deformgrad_template(i+1).plot_dudy_cur_formatted = plots_dispgrad_cur_prelim(i+1).plot_dudy; %#ok<AGROW> % lena bartell edit
                deformgrad_template(i+1).plot_dvdx_cur_formatted = plots_dispgrad_cur_prelim(i+1).plot_dvdx; %#ok<AGROW> % lena bartell edit
                deformgrad_template(i+1).plot_dvdy_cur_formatted = plots_dispgrad_cur_prelim(i+1).plot_dvdy; %#ok<AGROW> % lena bartell edit
            end

            % Store info:
            for i = 0:length(obj.current)-1
                obj.data_dic.strains(i+1) = strains_template(i+1);
                obj.data_dic.deformgrad(i+1) = deformgrad_template(i+1);
            end
        ...
        % This function updates state text depending on what is loaded
            ...
            % Strains:
            if (~isempty(obj.data_dic.straininfo) && ~isempty(obj.data_dic.strains) && ~isempty(obj.data_dic.deformgrad) )
